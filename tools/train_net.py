     ...
     #       boxRecord.shape, yVecs.shape))
        ...
        for i in tqdm(range(len(unlabeledidx))):
            ...
            for j,box in enumerate(boxRecord[i]):
                ...
                cls_loss_sum += loss
                # choose v by loss
                v, v_val = judge_v(loss, gamma, clslambda)
                # print('v_val:{}'.format(v_val))
                # ss process 
                if v:
        ...
        logging.info('last model name :{},AL amount:{}/{},al_factor:{},SSM amount: {}/{},ss_factor:{}'.format(pretrained_model_name,alamount,initial_num,al_factor,ssamount,initial_num,ss_factor))
